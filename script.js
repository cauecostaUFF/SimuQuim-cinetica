...
class Ball{
    ...
    changeVel(vel){
        this.vel.x = this.orig_vel.x*(1+vel/10)
        this.vel.y = this.orig_vel.y*(1+vel/10)
    }
...
function newBall(col){
    B[cont] = new Ball(x,y,ballRadius,(Math.random()*10 - 5)*(1+vel/10) ,(Math.random()*10 - 5)*(1+vel/10),col);
    cont +=1;
}
...
function draw() {
    ...
    for (let i = 0; i<B.length; i++){
        ...
        if (B[i].pos.y + B[i].vel.y > canvas.height - ballRadius || B[i].pos.y + B[i].vel.y  < ballRadius) {
...
document.getElementById("increaseT").addEventListener("click", function () {
    vel += 1; document.getElementById("decreaseT").disabled = false;
    if (vel<=5){
       for (let i = 0;i<B.length;i++){
            B[i].changeVel(vel);
        }
...
document.getElementById("decreaseT").addEventListener("click", function () {
    vel -= 1; document.getElementById("increaseT").disabled = false;
    if (vel>=-5){
            for (let i = 0;i<B.length;i++){
            B[i].changeVel(vel);
        }
...
function coll_det_bb(b1, b2){
    if(b1.r + b2.r >= b2.pos.subtr(b1.pos).mag()){
        if (Math.random()>0.9-(vel/10) && ((b1.color=="blue" && b2.color=="green")|| (b1.color=="green" && b2.color=="blue"))){
            b1.status = false;
            b1.r = 0;
            b2.color = "red";
            b2.r = 15;
